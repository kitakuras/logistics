...
import com.zz.pojo.OrderDetailExample;
...
public class OrderServiceImpl implements IOrderService {
	...
	public void getAddOrUpdateInfo(Model m, Integer orderId) {
		...
		if (orderId == null) {
			...
			if (orders != null && orders.size() == 1) {
				...
				// 到达国家
				BasicData destArea = basicDataService.queryById(order.getDestAreaId());
				// 付款方式
				...
				// 订单内容
				OrderDetailExample ex = new OrderDetailExample();
				ex.createCriteria().andOrderIdEqualTo(order.getOrderId());
				List<OrderDetail> orderDetails = orderDetailMapper.selectByExample(ex);

				List<BasicData> warehouse = basicDataService.queryBase(Constant.BASIC_WAREHOUSE);
				User use = (User) SecurityUtils.getSubject().getPrincipals().getPrimaryPrincipal();
				...
				// 到达国家
				m.addAttribute("destArea", destArea);
				m.addAttribute("paymentMethod", paymentMethod);
				m.addAttribute("transMethod", transMethod);
	...
	public boolean update(OrderDto dto) {
		...
		try {
			...
			orderMapper.updateByPrimaryKeySelective(dto);
			return true;
		} catch (Exception e) {
			e.printStackTrace();
			return false;
