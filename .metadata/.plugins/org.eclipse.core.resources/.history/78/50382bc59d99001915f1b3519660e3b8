...
<script type="text/javascript">
	...
	function goDelete(baseId){
		if(window.confirm("是否删除")){
			location.href="/customer/delete?id="+baseId;
		}
	}
</script>
			...
			<tbody>
				<c:forEach items="${list }" var="c">
						<tr>
							...
							<td>${c.customer.cSex?"男":"女"}</td>
							<td>${c.customer.email }</td>
							<td>${c.customer.idCard }</td>
							<td>${c.customer.mobilePhone }</td>
							<td>${c.customer.remark }</td>
							<td>${c.realName }</td>
							<td>${c.commonInterval }</td>
							<td><a
								href="/customer/goAddOrUpdate?id=${c.customer.customerId }"
								class="tablelink">修改</a> <a href="javascript:void(0)"
								onclick="goDelete(${c.customer.customerId })" class="tablelink">
									删除</a></td>
						</tr>
				</c:forEach>
			</tbody>
